...
class TotalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(label_path):
            ...
            # Extract raw image (x, y, w, h)
            boxes_[:, 1] = ((x1 + x2) / 2) / padded_w
            boxes_[:, 2] = ((y1 + y2) / 2) / padded_h
            boxes_[:, 3] *= w_factor / padded_w
            boxes_[:, 4] *= h_factor / padded_h
            ...
            targets_ = torch.zeros((len(boxes_), 6))
            targets_[:, 1:] = boxes_
        ...
        return img_path, img, original_img, targets, targets_

    def collate_fn(self, batch):
        paths, imgs, original_img, targets, targets_ = list(zip(*batch))
        # Remove empty placeholder targets
        targets = [boxes for boxes in targets if boxes is not None]
        targets_ = [boxes for boxes in targets_ if boxes is not None]
        # Add sample index to targets
        for i, boxes in enumerate(targets):
            boxes[:, 0] = i
        ...
        imgs = torch.stack([resize(img, self.img_size) for img in imgs])
        self.batch_count += 1
        return paths, imgs, original_img, targets, targets_
